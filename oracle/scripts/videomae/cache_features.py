#!/usr/bin/env python
"""Phase A — extract frozen-encoder features for VideoMAE-K400 head finetuning.

For each row in ``--train-csv``, loads the matching MSS sidecar from ``--mss-run-dir``,
builds the cut+concat input via ``oracle.scripts.vjepa2.adapter.build_input``
(``protocol="repack"``), runs the frozen encoder, and saves the pooled pre-head
feature ``last_hidden_state.mean(dim=1)`` to a per-shard ``.pt`` cache.
...
videos already in any existing ``shard_*.pt`` are skipped.

Output:
    <output-dir>/shard_<NN>.pt   — {"video_ids": [...], "features": (N,D), "labels": (N,)}
    <output-dir>/cache_metadata.json — args, model, mss run, label map, hashes
"""
...
from oracle.scripts.videomae.inference import VideoMaeWrapper
from oracle.scripts.vjepa2 import selectors as sel
from oracle.scripts.vjepa2.adapter import build_input

LOG = logging.getLogger("videomae.cache_features")

DEFAULT_MODEL_ID = "MCG-NJU/videomae-large-finetuned-kinetics"
...
def _load_existing_video_ids(output_dir: Path) -> set[str]:
    """Union of video_ids across all existing shard files in output_dir."""
    seen: set[str] = set()
...
def _save_shard(
    ...
    blob = {
        ...
        "features": torch.stack(features, dim=0),     # (N, D) float32
...
def main(argv=None) -> int:
    ...
    LOG.info(
        ...
        args.shard_id, args.num_shards, len(rows), len(all_rows), args.device, args.model_id,
    )
    ...
    LOG.info("Loading wrapper %s on %s …", args.model_id, args.device)
    wrapper = VideoMaeWrapper.from_pretrained(args.model_id, device=args.device)
    model = wrapper.model
    model.eval()
    encoder = model.videomae  # frozen
    id2label = model.config.id2label
    label_norm = {v.replace("[", "").replace("]", "").strip().lower(): int(k) for k, v in id2label.items()}

    # Resume: preload anything we've already written this run so we keep appending.
    ...
    # Batch by repeatedly accumulating (row, pixel_values, label_id) and flushing in groups.
    buf_rows: list[dict] = []
    ...
    def flush_batch():
        ...
        with torch.inference_mode():
            ...
            pooled = out.last_hidden_state.mean(dim=1).float().cpu()  # (B, D)
    ...
    for r in todo:
        ...
        # Resolve ground-truth class id.
        gt_norm = r["label"].replace("[", "").replace("]", "").strip().lower()
        ...
        # Load MSS sidecar + extract kept segments via vlm_selected (label=="important").
